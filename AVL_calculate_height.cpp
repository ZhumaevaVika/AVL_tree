...
void calculate_height(int (&array)[N], Node* &root, int sizes_number, int size_step, int rep_number){
    remove("height(n).txt");
    std::ofstream out;
    out.open("height(n).txt", std::ios::app);
...
int main(){
    int array[N];
    Node* root = nullptr;
    calculate_height(array, root, 100, 2, 1000);
}